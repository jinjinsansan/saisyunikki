...
import { Search, Calendar, Filter, Eye, MessageCircle, TrendingUp, Users, LogOut, RefreshCw, Shield } from 'lucide-react';
import { getAllDiaryEntries, getAllUsers, getStatistics, logoutCounselor, getCounselorInfo } from '../lib/sakuraApi';
...
const SakuraAdminDashboard: React.FC = () => {
  ...
  const counselor = getCounselorInfo();
  const emotions = [
  ...
  const loadLocalData = () => {
    ...
    if (savedEntries) {
      ...
      const todayCount = formattedEntries.filter((entry: any) => entry.date === today).length;
      const urgentCount = formattedEntries.filter((entry: any) => 
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 font-jp-normal">さくらサーバーからデータを読み込み中...</p>
        </div>
      </div>
    );
  }
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600 font-jp-normal">
                  {counselor?.name || 'カウンセラー'}さん - さくらサーバー
                </p>
              </div>
            </div>
            <div className="flex items-center space-x-4">
...
export default SakuraAdminDashboard;